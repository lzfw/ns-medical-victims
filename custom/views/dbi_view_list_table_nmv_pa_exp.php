...
class View_List_Table_NMV_PA_Exp extends View_List {
	...
	static public function create () {
		...
		switch (func_num_args()) {
			case 1: return new View_List_Table_NMV_PA_Exp ($args[0]);
		}
	}
	...
	public function get_HTML ($results) {
		...
		if ($results->num_rows>0) {
		    ...
		    $html .= '<th>Surname</th><th>First Names</th><th>ID</th><th>Born</th><th>Birth Country</th><th>Nationality (1938)</th><th>Ascribed Ethnic Group</th><th>Start Date Experiment<br>(D.M.Y)</th><th>Options</th>';
			// buttons view, edit, delete connection victim-experiment
			while ($item = $results->fetch_object()) {
					$options = '';
	        $options .= createSmallButton('View Prisoner Assistant',"nmv_view_victim?ID_victim=$item->ID_victim",'icon view');
	        $options .= createSmallButton('View Prisoner Assistant - Experiment',"nmv_view_prisoner_assistant_experiment?ID_pa_exp=$item->ID_pa_exp",'icon view');
