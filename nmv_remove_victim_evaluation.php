...
function get_evaluation($dbi, $id) {
    $sql = "SELECT *
        FROM nmv__evaluation
        WHERE ID_evaluation = ?";
    ...
    $evaluation = $result->fetch_object();
    return $evaluation;
}

function breadcrumb($dbi, $item, $record_id) {
    $victim = get_victim($dbi, $item->ID_victim);
    $evaluation = get_evaluation($dbi, $item->ID_evaluation);
...
function prompt($dbi, $item) {
    ...
    $victim_evaluation =
        htmlspecialchars($evaluation->ID_evaluation, ENT_HTML5).' - ' . 
        htmlspecialchars($victim->first_names, ENT_HTML5).' '.
        htmlspecialchars($victim->surname, ENT_HTML5);
    return 'Remove Victim Evaluation '.': "<em>'.$victim_evaluation.'</em>".';
}
...
remove_record('ID_evaluation', 'ID_evaluation', 'nmv__evaluation',
    'victim evaluation', 'nmv_view_victim', 'nmv_view_victim',
